...
- (void)reloadOrderTable{
    ...
    [self.sellingSummaryTableView reloadData];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        ...
        if (![[sellingOrderDict valueForKey:@"number"] isEqual:[NSNull null]]) {
            ...
            cell.phoneLabel.text = [NSString stringWithFormat:@"Acct: -%@",phoneNo];
        ...
        cell.cashDepositLabel.text = [NSString stringWithFormat:@"Verified Deposits: $%.02f",depositAmount];
        ...
        cell.totalDashLabel.text = [NSString stringWithFormat:@"Available %@: %@ (%@ %@)",WOCCurrencySpecial,totalDash,stringNum,WOCCurrencySymbolMinor];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 3) {
        UIView *headerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.sellingSummaryTableView.frame.size.width, 50)];
